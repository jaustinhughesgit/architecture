...
- The clean owner-local protected Compute layer admits only compiler-declared requirements that are consumed by a bounded formula. The first installation reconciles safe selector metadata to one exact protected binding; repeat runs use the saved binding/asset IDs and program hash. Only the protected worker decrypts, injects typed values, executes JPL, and formats a transient answer. Ordinary traces retain a value-free receipt; the server rejects the local execution plane. See [decision 0071](../decisions/0071-minimized-owner-local-protected-compute.md).
...
In the clean Phase 4A implementation, this policy applies only to an exact owner-local protected binding in the dedicated worker; the POC's remote placeholder/hydration replay is not inherited. The current clean prompt/controls provide direct one-use or timed presentation and no provider execution. Recipient hydration and trusted-server injection require their own later delivery contracts.
