...
A second loop remained because server hydration treated an older globally confirmed Path as authoritative and overwrote the newly installed compatible revision. AWS commit `3daea41a` changes foundation merging to prefer a quality-approved Path satisfying the required revision, whether it came from local installation or identity storage, over an incompatible confirmed copy. This preserves local-first execution while the confirmed foundation catches up. Asset phase: `phase80-foundation-merge-fix-1`; targeted suite: 64/64.
