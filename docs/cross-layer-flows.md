...
The active v1 contract publishes ordinary, non-protected relation components to participant-scoped server audiences. Compute verifies workspace ownership, resolves only exact unique public user handles, and returns authoritative IDs that the browser applies across its structured local state. Browser temporary IDs are monotonic allocation identities: snapshot load and acknowledgement remap reserve all historical local IDs before later mutations, and every explicit local entity materializer delegates allocation to that graph-owned counter rather than inferring availability from a canonicalized snapshot. Compute rejects an incompatible reuse of a principal-local ID so a new object cannot overwrite an older canonical node even if a client violates that invariant. Relation publication is content-aware, so changing the object behind an existing relation ID is synchronized; alias-only node changes republish one incident relation as their carrier. Hydration reads only the authenticated principal's audience and maps that principal's server entity to local `speaker`. A public workspace also publishes current-speaker-connected components to its server-derived public profile audience. Before a named question executes locally, the browser may send the exact proper-person label for Compute to resolve and hydrate; it cannot choose the target principal or audience, and the remote user is not mapped to local `speaker`. Publication is asynchronous and retryable, so connectivity never delays a local mutation.
...
For a supported ordinary current-property transition, the model supplies one frozen semantic answer and trusted code derives the capability contract directly from it. The exact property/value domain/effect are not copied through a second model response. Owner-local protected calculations retain their separately validated presentation proposal until that complete contract moves into the semantic schema. See [decision 0078](../decisions/0078-one-semantic-response-compiles-ordinary-transition-contracts.md).

Convert discovery is semantic-first. For a build decision, the model must state what source answers the request, the operation, the browser input, the output, and—when applicable—the ContextDB subject/property address before it declares the capability contract. The browser supplies a separate bounded `authoringContext` with up to 20 non-protected recent inputs and at most 120 browser-proven Essence rows; `relevantItems` stays empty, protected entries and full graph snapshots stay local, and the evidence is persisted with a background job. Compute reconciles the generated contract against the frozen answer plan before EntityPlan/JPL construction. A current-speaker possessive is an address owner, not an ordinary `user` value; a matching deictic pseudo-input may be repaired into the planned browser-resolved property input, while an explicitly declared or genuinely non-deictic person input is preserved. A current ordinary value may prove that an address is available, but it is removed from generated defaults, constants, utterance annotations, capability identity, and JPL literals. Convert authoring cannot resolve to `not_compute` merely because the eventual invocation reads local ContextDB data. A disagreement receives one bounded background semantic correction and then fails closed. Every accepted operation has a nonempty answer template before browser Path installation. See [decisions 0044](../decisions/0044-semantic-answer-plans-precede-compute-contracts.md) and [0045](../decisions/0045-convert-discovery-receives-bounded-ordinary-authoring-context.md).
...
For an ordinary sanitized provider write, the API returns the committed value-free provider-effect proof with the typed Compute result. The browser commits both to the exact pending interaction and installation. Review selects the terminal Compute trace by interaction ID instead of displaying the local cold-miss rejection that preceded discovery. A new typed invocation form compiles another local Path onto the existing exact installation; it does not mint a second installation solely because the typed value changed.
