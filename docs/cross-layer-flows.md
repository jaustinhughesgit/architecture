...
An exhausted repair that carries an explicit local-graph classification is still a Path repair, not evidence that a new external compute app is needed. Message asks for repair consent. Approval selects the failed message in Edit with its Path signature, scoped ContextDB evidence, rejected candidates, and diagnosis, then prepares an LLM repair instruction for user review. If an otherwise compute-eligible miss has no local classification and the local compiler cannot represent it, the browser sends the preserved failure evidence through capability jurisdiction once. A build/reuse decision continues through Compute; `not_compute` returns to the original Edit repair. Declining changes nothing.
