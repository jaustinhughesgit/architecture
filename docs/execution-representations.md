...
Clean ArrayLogic v1 is a sequential workflow of exact capability/version/operation steps with typed references to earlier outputs. It is not parallel execution. Its released Phase 3 compensation policy is explicitly `none`. Clean Shorthand remains a bounded internal build/patch representation and is not required to invoke a capability. Approved JavaScript adapters are the future escape hatch for behavior outside bounded JPL; they are prebuilt, immutable, digest-addressed, authority-classified artifacts, never packages dynamically installed during an invocation. Adapter execution remains fail-closed until Phase 4 supplies signing, review, and isolated execution cells. See [decisions 0055](../decisions/0055-compiler-owned-jpl-and-hybrid-compute-execution.md) and [0064](../decisions/0064-phase-3-releases-the-governed-non-protected-jpl-boundary.md).

This foundation is implemented and deployed at `1var.com` as release `d1b02dc0a28af71704465bd92091f54820dbef02`. The clean platform also has compact Position candidate routing, durable answer-first contract generation, content-addressed capability packages with external owner revocation, a model-free Binding Essence, a target-first middleware conformance runtime, durable browser-coordinated workflow effects/checkpoints, a 50-case model-promotion gate with sanitized token receipts, reviewed adapter conformance fixtures, and ordinary voice capture/encoding. The callable Phase 3 boundary is compiler-bounded non-protected JPL loaded from immutable S3 packages. The exact release passed 50/50 model promotion with zero critical failures, deployed voice acceptance, reset-gated production promotion, health checks, and rollback recording. Adapter execution cells, active entity-resolver middleware transport, non-`none` compensation, package signing, independent review, and protected references remain Phase 4. The sections below continue to document proof-of-concept behavior as migration evidence unless they explicitly say clean platform.
