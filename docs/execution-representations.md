...
Clean ArrayLogic is a sequential workflow of exact capability/version/operation steps with typed references to earlier outputs. It is not parallel execution. Compensation is either `none` or `explicit_reverse`; the latter requires an authored exact inverse for every step and executes only those inverses in reverse commit order. Clean Shorthand remains a bounded internal build/patch representation and is not required to invoke a capability. Approved JavaScript adapters are prebuilt, immutable, signed, digest-addressed, independently reviewed, authority-classified artifacts—never packages dynamically installed during an invocation. Fresh workers receive only admitted brokers, while native/large/CPU packages require a separate container executor. See [decisions 0055](../decisions/0055-compiler-owned-jpl-and-hybrid-compute-execution.md), [0064](../decisions/0064-phase-3-releases-the-governed-non-protected-jpl-boundary.md), and [0110](../decisions/0110-advanced-execution-is-signed-exact-and-explicitly-reversible.md).

The original Phase 3 JPL foundation is implemented and deployed at `1var.com` as release `d1b02dc0a28af71704465bd92091f54820dbef02`. Phase 5C now implements the signed adapter-cell runtime, exact browser/file/Compute middleware dispatcher, and authored reverse-compensation contract in `onevar-platform`; production promotion remains subject to its release gates. Bounded JPL remains the default and JavaScript remains a reviewed escape hatch. The sections below continue to document proof-of-concept behavior as migration evidence unless they explicitly say clean platform.
