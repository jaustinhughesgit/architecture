...
**Status (2026-08-24): In closeout.** The clean platform now has strict Compute/build/install/JPL/effect/ArrayLogic/adapter/middleware contracts; a trusted build-plan-to-JPL compiler and bounded interpreter; exact browser-local installations plus model-free Binding Essence rebinding; property-scoped current relations; durable two-stage answer-first Convert generation; compact lexical plus Position discovery; content-addressed S3 capability packages verified independently by browser and server; transient ordinary AudioWorklet/encoder-worker transcription that rejoins the typed dispatcher; atomic browser effect application; and clean two-browser car-wash acceptance that reuses its installed Path with zero model/discovery calls. Durable browser-coordinated ArrayLogic persists exact step mappings, invocation identities, retries, typed outputs, effect receipts, cancellation, and atomic Context checkpoints across reload. Immutable packages have a separate owner-revocable release record enforced by discovery, download, and invocation. A deployed 50-case, ten-domain authoring promotion gate requires 98% with zero critical failures and retains sanitized token receipts. The production boundary is ordinary non-protected compiler-bounded JPL; new DynamoDB rows retain only the manifest and routing metadata, a bounded migration reader ignores a legacy adjacent program, and exact executable packages are loaded and verified from S3. Adapter fixtures and target-first middleware remain deterministic conformance foundations. JavaScript adapter execution, non-`none` compensation, active middleware transport, package signing, independent review, and protected references move behind Phase 4 governance and remain fail-closed. Phase 3 is not complete until development measurement, voice acceptance, production promotion, health checks, and rollback evidence pass. See [decisions 0055](../decisions/0055-compiler-owned-jpl-and-hybrid-compute-execution.md) through [0064](../decisions/0064-phase-3-releases-the-governed-non-protected-jpl-boundary.md).
