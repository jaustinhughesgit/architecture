...
- Phase 5A development candidate implemented: a production-only double-entry contract/repository separates `credit_micro` and `usd_cent` from non-cash conformance credits; Stripe-hosted Checkout fulfills through either an exact signed webhook or an API retrieval and exact verification of the returned Checkout Session, with both transports converging on one idempotent journal; a versioned promotional campaign can issue one exact user's production credits with no false cash posting; hosted Connect onboarding stores no bank details; marketplace install/refund and protected-provider reservation/settlement post exact journals; a versioned 3% user-transfer policy splits publisher payable from platform revenue and freezes the refundable split; and explicit whole-unit accrual precedes exact Stripe Connect transfer. Raw cost meters remain independent from 140% platform/payment, 120% model, and 110% 1var-managed-provider policies. The two-lane successor gives standard server interactions (at most five seconds and 5,000 OpenAI tokens) the preceding 5 AM-to-5 AM block's frozen aggregate rate and itemizes anything exceeding either boundary. Standard settlements store compact receipts and update one of 256 shards in the same credit-ledger transaction; heavy costs never enter the average. Incomplete cost coverage keeps the explicit sandbox bootstrap rate rather than publishing a deceptively low estimate. Ordinary server JPL and Convert authoring are first adopters; local-only work is free. Production stays disabled until Stripe approves 1var's exact funds flow. Authoritative AWS resource allocation, remaining transport adoption, and Stripe refund/dispute/payout reconciliation remain incomplete.
...
**Current candidate (2026-08-28):** Phase 5A production-billing contracts, sandbox Checkout/Connect integration, marketplace/provider journals, versioned promotional issuance, explicit publisher cash accrual, Connect transfers, and two-lane daily interaction pricing are implemented as a development candidate but remain unapproved for production. Phase 5B adds minimal non-secret protected-approval summaries, conditional recipient-key rotation, a fail-closed browser-local protected speech path using a verified local language pack, zero-knowledge multi-device recovery, and exact current-version recipient removal. Recipient groups, portable packaged local ASR, general protected files/ArrayLogic/schedules, chunked large-store recovery, and browser-absent owner-local execution remain fail-closed.
