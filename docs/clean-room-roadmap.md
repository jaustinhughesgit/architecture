...
- Scale-prove and promote the development-proven Phase 4F governed-schedule foundation: exact capability/version/installation/package/JPL/input/price snapshots, runtime protected reauthorization, deterministic idempotency, shared due-index dispatch, SQS retry/DLQ, pause/resume/cancel, and browser-acknowledged results. The current repair candidate must additionally prove that the same exact relation may advance versions between preparation and delivery, that delivery refreshes only that ID and rechecks the effect precondition, that one pending failure cannot block later inbox results, and that server completion is visibly distinct from pending browser delivery. Then add calendar rules, edit/version migration, downstream channels, and ArrayLogic automation scheduling.
