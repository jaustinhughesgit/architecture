...
- Phase 5A development candidate implemented: a production-only double-entry contract/repository separates `credit_micro` and `usd_cent` from non-cash conformance credits; Stripe-hosted Checkout can mint credits only after exact signed webhook settlement; hosted Connect onboarding stores no bank details; marketplace install/refund and protected-provider reservation/settlement post exact journals; configured platform fees split publisher payable from platform revenue; and explicit whole-unit accrual precedes exact Stripe Connect transfer. Production stays disabled until Stripe approves 1var's exact funds flow. Test-credit operation records remain transitional, and durable cost meters plus Stripe fee/dispute/payout reconciliation remain incomplete.
