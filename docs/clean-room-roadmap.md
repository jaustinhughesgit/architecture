...
- Phase 5A development candidate implemented: a production-only double-entry contract/repository separates `credit_micro` and `usd_cent` from non-cash conformance credits; Stripe-hosted Checkout fulfills through either an exact signed webhook or an API retrieval and exact verification of the returned Checkout Session, with both transports converging on one idempotent journal; hosted Connect onboarding stores no bank details; marketplace install/refund and protected-provider reservation/settlement post exact journals; a versioned 3% user-transfer policy splits publisher payable from platform revenue and freezes the refundable split; and explicit whole-unit accrual precedes exact Stripe Connect transfer. Raw model-cost meters remain independent from a versioned projection of 140% platform/payment, 120% model, and 110% 1var-managed provider cost. Checkout now records the exact Stripe balance-transaction fee, provider receipts separate 1var-paid USD cost from app credits, source coverage is explicit, and AWS resources carry allocation tags. Production stays disabled until Stripe approves 1var's exact funds flow. AWS cost-tag activation, CUR 2.0 ingestion/allocation, and Stripe refund/dispute/payout reconciliation remain incomplete.
