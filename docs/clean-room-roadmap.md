...
**Status (2026-08-24): Implemented and deployed.** Release `6d55c528f9e8b11edc6b282639d4c29e387bb6b9` projects `essence | convert` and `ordinary | protected` as four stable lanes. Hard stop is a Convert boundary rather than a mode. Typed protected sentinels are discarded before ContextDB, authoring, terminal history, or network. One AudioWorklet tags PCM with exact lane, segment index, and sample frames; protected entry is immediate, protected exit has a 100 ms guard, and any mixed-trust held session makes the complete capture fail closed without remote transcription. Protected Convert blocks its draft. Development workflow 32798581498 and production workflow 32799020746 passed reset-gated browser acceptance and the deployed four-lane voice gate; a fresh live `1var.com` entity also proved protected typed input absent from the transcript and ordinary Essence synchronized. Local ASR, encrypted asset creation, protected references, grants, and governed execution remain Phase 4A work. See [decision 0065](../decisions/0065-four-lane-input-trust-surface.md).
