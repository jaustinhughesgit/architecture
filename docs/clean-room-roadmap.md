...
- Implemented in the Phase 4G candidate: immutable KMS package signing and build provenance, passkey-authenticated-account publisher verification, compiler review state, compatibility, natural app/profile-addressed collaboration revocation, staged rollout, and lazy per-account rollback.
- Implemented in the Phase 4G candidate: exact installations, licenses, concurrency-safe usage receipts, zero-use refunds, publisher attribution, collaboration grants, licensed forks, and discoverable-passkey same-account release-delta synchronization independently of user data and protected assets.
