...
**Current candidate (2026-08-27):** Phase 5A production-billing contracts, sandbox Checkout/Connect integration, marketplace/provider journals, explicit publisher cash accrual, Connect transfers, and two-lane daily interaction pricing are implemented but the pricing successor is not yet deployed or production-approved. Phase 5B adds minimal non-secret protected-approval summaries, conditional recipient-key rotation, and a fail-closed browser-local protected speech path using a verified local language pack. Multi-device protected recovery, recipient groups, portable packaged local ASR, general protected files/ArrayLogic/schedules, and browser-absent owner-local execution remain fail-closed.
