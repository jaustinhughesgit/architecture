...
**Status (2026-08-24): In progress.** The clean platform now has strict Compute/build/install/JPL/effect/ArrayLogic/adapter/middleware contracts; a trusted build-plan-to-JPL compiler and bounded interpreter; exact browser-local installations plus model-free Binding Essence rebinding; property-scoped current relations; durable two-stage answer-first Convert generation; compact lexical plus Position discovery; content-addressed S3 capability packages verified independently by browser and server; transient ordinary AudioWorklet/encoder-worker transcription that rejoins the typed dispatcher; atomic browser effect application; and clean two-browser car-wash acceptance that reuses its installed Path with zero model/discovery calls. Durable browser-coordinated ArrayLogic persists exact step mappings, invocation identities, retries, typed outputs, effect receipts, cancellation, and atomic Context checkpoints across reload. Immutable packages have a separate owner-revocable release record enforced by discovery, download, and invocation. A 50-case, ten-domain authoring promotion gate requires 98% with zero critical failures. Reviewed adapter fixtures cover CommonJS, ESM, dual, async, transitive, file, network, state isolation, timeout, and container routing. Target-first middleware deterministically stops on the selected child's first response. This is the permanent non-protected Compute foundation, not Phase 3 completion. Production adapter cells, active middleware adoption by entity resolution, production model measurement, real-device voice acceptance, deployment, and the full release gates below remain open. Package signing, independent review, and marketplace governance remain Phase 4. See [decisions 0055](../decisions/0055-compiler-owned-jpl-and-hybrid-compute-execution.md) through [0063](../decisions/0063-target-first-entity-middleware-order.md).
...
- Adopt Entity Middleware v1 across active planes: deterministic selected-target-to-root `extend` lineage, per-node authorization, sequential `pass`/`respond`/`fail`, and immediate stop on the first response or failure.
