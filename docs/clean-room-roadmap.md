...
- Ordinary current-property transition Convert now derives its contract deterministically from one frozen semantic response; reviewed provider actions use catalog-owned one-response expansion, while protected calculations retain a separately validated presentation proposal until their first schema is complete. See [decision 0078](../decisions/0078-one-semantic-response-compiles-ordinary-transition-contracts.md).
- Release `648ccf8605c35c40c28cf9735ddb6bcd6f4e3437` deployed this narrower authoring boundary. Development workflow [33008411751](https://github.com/jaustinhughesgit/onevar-platform/actions/runs/33008411751) proved the exact vehicle-wash request from reset with one provider response, deterministic contract/JPL compilation, exact installation binding, independent `dirty -> clean` verification, and governed scheduled execution.
...
**Status (2026-08-27): Phase 4A through Phase 4G are implemented and development-proven.** Phase 4A and 4B provide browser-only protected spans and recipient-specific zero-knowledge sharing. Phase 4C provides exact hardware-gated owner-local protected calculation. Phase 4D explicitly promotes one credential version for a database-blind isolated executor under exact server-verified WebAuthn authority, and release `bc0ff4861b143797e4db6f5412837f850a2e3356` admits one exact read-only WeatherAPI operation with bounded ordinary output. Phase 4E.1 release `8670955f9d6040132a2057740816ed442764a822` adds catalog-owned provider name/domain/kind/environment/scope/help provenance, provider-specific local selectors, publish-time credential requests, vague-label rejection, rotation, and revocation while retaining exact IDs as authority. Phase 4E.2 adds executor-to-browser encrypted provider results; 4E.3 adds exact provider-write idempotency, pre-commit cancellation, recovery, and effect proof; 4E.4 adds non-cash test-credit reservation, settlement, and full failure refund with separated provider/publisher/platform economics. Release `bf1560c6d9cd7623f2ecc3960f5c354b2c045bd6` passed immutable workflow `32997983235` and a separate 42.8-second global-reset live-model canary covering all four lifecycle fixtures. Phase 4F adds exact schedules, shared dispatch, browser-authoritative four-mode sync, natural app menus, focused immutable ArrayLogic improvement, and generic local record/aggregate primitives. Phase 4G release `823daa3be700f16f17a5ff82cae7394289a2df2c` and workflow `33046660355` add signed portable releases, passkey-authenticated-account publishers, automatic compiler review, exact licenses/installations/pricing limits, discoverable-passkey same-account library deltas, cross-user use and write grants, licensed forks, upgrade/rollback/name-addressed revocation, refunds, attribution, and abuse intake without transferring foreign Paths or protected authority. A separate 30.7-second clean-reset canary proved two-account build/sign/share/install/invoke/recover/revoke behavior, and the final inventory returned zero. The admitted write and paid provider operations remain no-network fixtures. Real provider writes, protected owner-local scheduled work while the browser is absent, request notifications, multi-device protected recovery, recipient groups, segmented local protected ASR, Stripe purchase/payout, creator earnings, verified email/recovery, advanced publisher verification/moderation, production Phase 4F/4G promotion, and million-user scale proof remain fail-closed. See [decisions 0068](../decisions/0068-browser-only-protected-span-sealing.md) through [0084](../decisions/0084-account-passkeys-are-not-protected-authority.md).
...
- Make every provider-credential request catalog-owned and provider-specific before broad provider admission. The user-facing request must show the provider's display name, canonical domain, credential kind, intended scope/environment, and an acquisition/help link when one exists; a generic label such as `weather API key` is insufficient when multiple issuers could satisfy it. Binding and authorization continue to use exact provider, adapter, operation, requirement, and credential IDs—the displayed domain is provenance for the person, not identity or an execution security boundary. If more than one compatible local Protected Asset exists, require an explicit local choice without revealing either value.
...
- Implemented and development-proven in Phase 4G: immutable KMS package signing and build provenance, passkey-authenticated-account publisher verification, compiler review state, compatibility, natural app/profile-addressed collaboration revocation, staged rollout, and lazy per-account rollback.
- Implemented in the Phase 4G development candidate: exact installations, licenses, concurrency-safe usage receipts, refund requests after use with exact-publisher approval/denial and resumable settlement, publisher attribution, collaboration grants, licensed forks, and discoverable-passkey same-account release-delta synchronization independently of user data and protected assets. Platform dispute/appeal and forced-refund authority remains Phase 5 work.
- Phase 5A development candidate implemented: a production-only double-entry contract/repository separates `credit_micro` and `usd_cent` from non-cash conformance credits; Stripe-hosted Checkout fulfills through either an exact signed webhook or API retrieval, with both transports converging on one idempotent journal; hosted Connect onboarding stores no bank or tax values; marketplace/refund/provider operations post exact journals; and a versioned 3% transfer policy freezes the refundable split. Raw costs remain separate from 140% platform/payment, 120% model, and 110% managed-provider projections. Standard interactions use the preceding completed 5 AM block's frozen aggregate rate and heavy work remains itemized. Phase 5.1A adds authoritative tagged AWS blocks plus a durable signed Stripe event inbox and exact refund/dispute/transfer/payout reconciliation. Phase 5.1B adds an immutable payout policy, atomic rolling-reserve lots, minimum withdrawals, Stripe-hosted identity/tax readiness, exact connected-balance transfers, signed bank-payout evidence, and explicit payout reversal. Production stays disabled until Stripe and legal/finance approve 1var's exact funds flow and operations supplies a production policy. Remaining transport adoption and heavy AWS attribution remain incomplete.
- Meter model, Compute, payment, provider, storage, bandwidth, and platform charges per invocation; record raw cost, user price, publisher earning, platform revenue, transfer fee, and margin without storing protected payloads. Cost-based production-credit debits must wait for authoritative source coverage and exact reservation/settlement journals.
- Scale-prove and promote the development-proven Phase 4F governed-schedule foundation: exact capability/version/installation/package/JPL/input/price snapshots, runtime protected reauthorization, deterministic idempotency, shared due-index dispatch, SQS retry/DLQ, pause/resume/cancel, and browser-acknowledged results. Release `694508d5f35b7890604c03720a4c94977255c40b` and workflow [33011169676](https://github.com/jaustinhughesgit/onevar-platform/actions/runs/33011169676) additionally prove that the same exact relation may advance versions between preparation and delivery, delivery refreshes only that ID and rechecks the effect precondition, one pending failure cannot block later inbox results, and server completion is visibly distinct from pending browser delivery. Production promotion and scale proof remain; then add calendar rules, edit/version migration, downstream channels, and ArrayLogic automation scheduling.
- Implemented and deployed to development in `onevar-platform`: ordinary file entities support checksum-bound direct upload/download, immutable versions, exact recipient grants/revocation, retention metadata, and deterministic text-to-PDF generation with source/output receipts. Development workflow [33297907699](https://github.com/jaustinhughesgit/onevar-platform/actions/runs/33297907699) passed the full repository gate and 24 reset-gated deployed browser scenarios, including two-user versioning, PDF generation, and revocation. Protected files and executable packages remain separate. Public artifact marketplace pricing, richer formats, and protected-file server Compute remain future work.
...
**Current candidate (2026-08-28):** Phase 5.1B production-billing contracts, sandbox Checkout/Connect integration, marketplace/provider journals, promotional issuance, explicit publisher cash accrual, rolling reserves, governed withdrawals, signed payout reconciliation, and two-lane daily interaction pricing are implemented as a development candidate but remain unapproved for production. Phase 5B adds minimal non-secret protected-approval summaries, conditional recipient-key rotation, a fail-closed browser-local protected speech path using a verified local language pack, zero-knowledge multi-device recovery, exact current-version recipient removal, recipient groups, browser-only chunked protected files, and all-protected local ArrayLogic. Portable packaged local ASR, protected file Compute/PDFs, protected schedules, mixed-plane workflows, chunked large-store recovery, and browser-absent owner-local execution remain fail-closed.
