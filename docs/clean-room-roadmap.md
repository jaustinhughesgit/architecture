...
**Status (2026-08-25): The clean typed owner-local Phase 4A boundary is implemented and deployed in release `a73dcd6dfe44c5038ed3270ae3ab29b666ed66e9`.** Each protected span receives its own AES-256-GCM content key and opaque versioned reference; a dedicated worker wraps content keys under a non-extractable identity-scoped AES-256-KW key and atomically stores ciphertext plus the ordered ordinary/reference envelope. A bounded model-free compiler now binds an ordinary owner-quantity or self-property statement to that exact asset/version. Exact local questions remain masked until an independently scoped one-use or timed Reveal/Speak grant authorizes transient worker decryption. Rotation invalidates old grants; safe audit, revocation, reload, command parity, and reset are implemented. Protected plaintext remains excluded from ordinary ContextDB, models, Compute, network, terminal history, and review. Development workflow [32814141108](https://github.com/jaustinhughesgit/onevar-platform/actions/runs/32814141108) and production workflow [32814455645](https://github.com/jaustinhughesgit/onevar-platform/actions/runs/32814455645) each passed all 17 reset-gated deployed scenarios for the exact release. A separate fresh production entity proved masked query, one-use Reveal outside terminal history, and complete protected reset. Entity Use Binding v1 still rejects protected references. Phase 4B recipient ECDH sharing, Phase 4C trusted-server/provider injection, request notifications, and segmented local protected ASR remain fail-closed rather than inheriting owner-local authority. See [decisions 0068](../decisions/0068-browser-only-protected-span-sealing.md) and [0069](../decisions/0069-exact-owner-local-protected-context-and-presentation.md).
