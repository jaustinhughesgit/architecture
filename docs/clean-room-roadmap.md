...
- Implemented candidate in `onevar-platform`; deployment proof pending: ordinary file entities now support checksum-bound direct upload/download, immutable versions, exact recipient grants/revocation, retention metadata, and deterministic text-to-PDF generation with source/output receipts. Protected files and executable packages remain separate. Public artifact marketplace pricing, richer formats, and protected-file server Compute remain future work.
