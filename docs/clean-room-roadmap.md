...
- Phase 5A candidate implemented: a production-only double-entry contract/repository separates `credit_micro` and `usd_cent` from non-cash conformance credits; Stripe-hosted Checkout can mint credits only after exact signed webhook settlement; hosted Connect onboarding stores no bank details; deterministic journals cover reservation, spend, release, refunds, cost, publisher accrual, and transfers. Development sandbox deployment and acceptance remain pending, and production stays disabled until Stripe approves 1var's exact funds flow.
...
**Current candidate (2026-08-27):** Phase 5A production-billing contracts, sandbox Checkout/Connect integration, and exact ledger primitives are implemented but not yet deployed or production-approved. Phase 5B adds minimal non-secret protected-approval summaries to four-mode sync. Multi-device protected recovery/rotation, recipient groups, local protected ASR, protected files/schedules/ArrayLogic, and browser-absent owner-local execution remain fail-closed.
