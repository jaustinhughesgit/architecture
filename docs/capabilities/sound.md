...
The clean runtime proves the ordinary-response autoplay boundary by preparing one scoped reusable Web Audio context during any accepted user submission gesture. Once an exact ordinary answer exists, it may prepare transient hosted audio concurrently with remaining rendering, audit, and bookkeeping work. The audio cannot play until the same interaction releases an accepted result; a failed, clarified, cancelled, or interrupted interaction aborts and discards the prepared request. Incoming relays use an ordered device queue and exact relay-derived interaction identity. OpenAI speech is the normal ordinary route and device speech is the fallback. Protected synthesis remains local-only and never enters this hosted route.
