...
Terminal classification-only and identity-address values are presented inside their subject's facts rather than as extra possession points. The strongest active `name`, `nickname`, `alias` or `callsign` labels that same exact subject point while kind remains a helpful detail. An addressable described possession keeps its complete lexical label and remains its own object point. This is visual folding only: original entity IDs, records and relations remain unchanged, so historical property-shaped name graphs need no migration. Separate same-name objects remain separate. Values used by another semantic relation or with outgoing structure remain inspectable points. The same rule applies only to already-authorized peer slices. Folded records do not create hidden relevance shortcuts between objects that share a kind or identity value. See [decision 0148](../../decisions/0148-named-possessions-use-one-exact-identity-object.md) and [decision 0150](../../decisions/0150-descriptive-possessions-create-nested-objects.md).
