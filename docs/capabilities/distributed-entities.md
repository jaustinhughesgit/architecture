...
When the question uses a possessive proper-person subject, the exact local invocation Path may specialize an entity input declared as `contextdb / speaker / <property>` to the hydrated named node for that wording. This is installation data, not a new Austin-specific entity contract: the reusable definition, operation, property, authorization boundary, and local-only value resolution remain unchanged. The diagnostic bundle retains the exact named-hydration result before discovery or replay.
