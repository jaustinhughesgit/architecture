...
- **Daily activity circles:** the five ray positions represent today through four days ago in the viewer's local timezone. App creation, an explicit person-initiated open, every successful app execution, and an accepted ordinary ContextDB mutation under the exact selected ray contribute exact interaction-deduplicated events. A non-app Context mutation uses a separate content-free ray aggregate and never fabricates a widget. Passive rendering, startup restoration, focus hydration, reads, rejected inputs, clarification prompts, viewport motion, and downloads create none. Empty days render nothing; activity older than the five-day window falls out of this projection without a cleanup job. Event-count floors render `1–3` small, `4–19` medium, and `20+` large; weighted unresolved severity may promote urgent work sooner. Circles do not own clicks, action drags, keyboard-button behavior, identity, or authority.
...
- **Center tap:** in the integrated Inspector V2, step backward through five local days (today, yesterday, two, three, four days ago, then today). Committing a ray selection starts at today. Five faint white orbit lines remain visible even for empty activity rows; the selected orbit and day label identify the lens. The standalone renderer retains the earlier widget-page fallback.
...
Rotating the wheel also projects the selected group's recent input/response cards beside the Sunburst. The projection remains briefly after release so it can be scrolled; scrolling pins the current projection instead of allowing the ambient fade. The expanded four-button input drawer renders the same typed feed rather than a second transcript. The browser retains a bounded recent working set, not a lifetime conversation archive: entries are category-tagged locally, capped, and age out, while current entity state remains in ContextDB and durable audit or financial receipts remain in their own governed stores. Protected plaintext is never admitted to this feed.

## Input and app presentation

The Inspector day lens uses a bounded browser-local index of exact ordinary entity/app references, installation/category, day key and last-use time, capped at 2,000 entries per owner and five visible local days. Accepted mutation receipts and their active relation endpoints, app attention and explicit Inspector opens populate it. Publication acknowledgements remap local references to canonical IDs. Explicit point opens add low-weight `entity.opened` attention; navigation/rendering never does. The newest 24 references prioritize currently owned or currently authorized scene candidates without recoloring them or inventing connections. Empty days state that no recent entities are available. This shows current entity state, not old snapshots, retains no transcript or protected plaintext, and cannot backfill earlier activity from counts alone. Cross-device history and inferred references for ordinary answers without exact receipts remain unimplemented.
...
Release `58e5440b23cb322e502a63e31b2e73df3b0a5459` reached that site through immutable GitHub Actions run `33414267285` without resetting test state. Complete repository verification and all 34 runnable CI browser tests passed before promotion; 11 environment-gated tests remained skipped. Focused browser acceptance additionally proved that a stationary center hold enters and releases the ordinary Essence voice lane and that selecting an app widget submits its registered open command immediately. CloudFront serves the exact homepage vector mark from `/assets/onevar-logo.svg` as a versioned S3 object, and the deployed Sunburst uses that mark inside the enlarged center control without the former instructional caption.
