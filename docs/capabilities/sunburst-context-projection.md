...
- **Daily activity circles:** the five ray positions represent today through four days ago in the viewer's local timezone. App creation, an explicit person-initiated open, and every successful app execution contribute exact interaction-deduplicated events, including a successful idempotent execution. Passive rendering, startup restoration, focus hydration, viewport motion, and downloads create none. Empty days render nothing; activity older than the five-day window falls out of this projection without a cleanup job. Event-count floors render `1–3` small, `4–19` medium, and `20+` large; weighted unresolved severity may promote urgent work sooner. Circles do not own clicks, action drags, keyboard-button behavior, identity, or authority.
...
The current local attention slice uses strict content-free events, exact-subject aggregates, retry deduplication, weighted severity, unresolved state, and read-time decay. It projects app creation, explicit person-initiated opens, focused-app improvement, and committed focused-app effects. Passive rendering, restoration, and focus hydration only register the widget and never change attention. Durable cross-device event projection from schedules, communications, requests, security, billing, and provider subsystems remains Partial.
