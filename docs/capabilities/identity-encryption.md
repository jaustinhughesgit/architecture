...
- Clean Phase 4C release `399c7deebe669b5eb684597087f0af637f243ceb` adds bounded owner-local protected numeric Compute. Compiler-owned requirements bind to exact local protected binding/asset IDs, the protected worker decrypts only declared inputs and runs hash-pinned JPL, the response remains transient, and durable traces contain only a value-free receipt. An unrelated protected asset is mechanically excluded. Exact WebAuthn-at-use for this plane is now implemented and awaits deployment evidence: enrollment grants nothing, assertions create exact once/timed authority, and binding rotation requires new authority. Trusted-server executor wrapping, broader local execution, and hardware gates for other protected modes remain incomplete.
