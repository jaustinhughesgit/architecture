...
Entities can have parent/child lineage. Entity middleware v1 resolves one deterministic owning lineage from the selected target through its nearest owning ancestors to the root, authorizes `use` at every node, invokes sequentially, and accepts only `pass`, `respond`, or `fail`. The first response or failure terminates the chain; cycles, ambiguous owning parents, inconsistent lineage, and depth overflow fail closed. The child is therefore an override or pre-handler and the parent is fallback middleware. Retry is never implicit. The runtime contract is implemented as a Compute foundation; active adoption by the browser, `fileWorker`, and Compute transports must preserve their trust boundaries. See [decision 0063](../decisions/0063-target-first-entity-middleware-order.md).
...
A clean app is an immutable bundle release over exact root/supporting capability releases, optional ArrayLogic, and one view projection. A user's menu entry is a separate local projection containing aliases, lifecycle, source, and granted permissions. Opening an app prioritizes its compatible installed entities without changing ownership or authority; invoking an app does not change the view. Resolution proceeds from an exact installed Path to the open app, the rest of the local menu, and only then authorized global discovery. Repeated plain Convert builds owned by one local user may retain separate immutable capability IDs and model-generated vocabulary. If and only if equal-scoring candidates are owner-built root-only apps, the most recently registered or explicitly opened build wins when the normalized contracts match or substantial local Convert requirement evidence proves the same repeated authoring request. Publisher, marketplace, unrelated-request, supporting-entity, and ArrayLogic differences remain true ambiguity. Focused analytics are authored as generic browser-local record/aggregate entities and sequenced after root success, never emitted as a fixed platform side effect. See [decision 0081](../decisions/0081-local-app-menu-and-browser-aggregate-composition.md).
