...
A reusable Compute operation may declare a bounded ContextDB effect without acquiring the graph. Compute returns a typed output; the browser resolves the spoken entity reference, proves one relation still has the manifest-declared current value, and applies the replacement through ordinary local mutation operations. Ambiguous or stale targets fail closed, and an already-applied replacement is idempotent. JPL access to governed server entities does not grant access to browser-local ContextDB. See [decision 0046](../decisions/0046-browser-applied-compute-context-effects.md).
