...
Inbound asynchronous synchronization preserves that browser authority. A server may durably commit a schedule occurrence or publisher-addressed analytics receipt, but it does not mutate browser ContextDB. An authenticated cursor delta brings authorized evidence to the active entity; scheduled effects still rejoin the exact local installation and commit there. Device-selectable light, medium, heavy, and real-time modes govern cost versus freshness, with one visible leader tab. Real-time push is only a constant change hint followed by the same cursor pull; it carries no graph, result, or protected value. Publisher analytics may describe exact capability execution and economics but must omit the invoking user and their data. See [decision 0079](../decisions/0079-browser-authoritative-sync-has-four-cost-modes.md).
