...
One remote interaction root has one production-pricing receipt and at most one corresponding debit. Authoring, JPL execution, reviewed providers, schedule occurrences, and remote ordinary transcription all join that root settlement boundary instead of maintaining parallel money paths. Operations at or below five seconds and 5,000 model tokens consume the frozen standard-lane rate. A settling operation that exceeds either threshold records itemized model usage plus an immutable AWS Lambda request, duration, memory, architecture, region, and response-byte rate card. Heavy server and model evidence remains visible but is excluded from the next standard-lane average so ordinary users do not subsidize exceptional work. Replays may recover the existing receipt, but cannot create another debit. Missing pricing, authoritative usage, or a root receipt conflict fails closed before a result or external effect is released.
