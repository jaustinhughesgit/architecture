...
| Data-defined local semantic entities | Implemented foundation | `aws` | Versioned operation templates compile locally with Path bindings. Personal and qualified-owner quantities share signed observation storage. Quantity, participant-event, vocabulary-neutral activity occurrence, relationship transition, selected-relation correction, and local trigger/effect policy entities exist. The relationship-transition contract separates an observed transition, object/reference identity, and a declaratively selected resulting state so cross-predicate meaning lives in reusable Path/entity data instead of runtime vocabulary. Correction provenance/version history, general ontology inference, state expiration/reversal, and formal contract schemas remain incomplete. |
| Typed Semantic Plan → Path synthesis | Implemented foundation | `aws` | Model-selected semantic operations and typed role bindings are compiled deterministically into matchers and catalog-owned executable transforms. Versioned support-binding derivations can expand one query plan into an atomic historical-repair/query transaction. Direct Semantic Plan API schema, broader dependency contracts, and complete capability-gap reporting remain work. See [decision 0011](../decisions/0011-typed-semantic-plan-path-synthesis.md). |
