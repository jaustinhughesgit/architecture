...
| Typed Semantic Plan → Path synthesis | Implemented foundation | `aws` | Model-selected semantic operations and typed role bindings are compiled deterministically into matchers and catalog-owned executable transforms. Versioned support-binding derivations can expand one query plan into an atomic historical-repair/query transaction. A bounded classification prerequisite can derive a supporting object's broader type from the current question and prove both Paths locally. Direct Semantic Plan API schema, broader dependency contracts, and complete capability-gap reporting remain work. See [decision 0011](../decisions/0011-typed-semantic-plan-path-synthesis.md) and [decision 0012](../decisions/0012-bounded-semantic-prerequisite-expansion.md). |
...
| Required and expanded Path learning | Partial | `aws` + model services | A cold miss builds a typed Semantic Plan that deterministic synthesis turns into a candidate Path; the browser then validates, installs, and replays it before graph execution. Candidate checks distinguish syntactic token coverage from typed semantic coverage. A query may expand through a data-declared support contract into an atomic paired repair that removes and replays a defective supporting statement in isolation. The first bounded semantic prerequisite supports specific-entity/general-type stitching. Auto-build separately permits paraphrase signatures. General ontology inference and broader dependency/capability-gap contracts remain incomplete. |
