...
| Browser-local canonical Essence generation | Implemented foundation | `aws` | Installed Paths materialize validated local transforms, while a structural browser compiler handles safe cold-miss event structures before Path construction. Browser execution does not fall back to model-generated Essence. Coverage and formal versioned Essence schemas remain incomplete. |
...
| Required and expanded Path learning | Partial | `aws` + model services | A cold miss with safe local Essence always builds and saves the required current Path. Auto-build separately permits proposals for additional paraphrase signatures with equivalent local Essence. Every candidate must compile and validate locally; reliable reusable-family repair is incomplete. |
