...
| Authorized test-environment reset | Implemented foundation | `testing` + `compute` | Client and server fail-closed gates exist; only explicitly configured non-production deployments and allow-listed authenticated test users may reset. Parallel isolation remains future work. |
