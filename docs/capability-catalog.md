...
| Browser-local canonical Essence generation | Implemented foundation | `aws` | Installed local Paths materialize the canonical Essence used for the current request. Browser execution does not fall back to model-generated Essence; uncovered language requires a local Path, clarification, or explicitly enabled Path learning. Formal versioned Essence schemas remain incomplete. |
...
| Automatic Path learning | Partial | `aws` + model services | The explicit Auto-build control permits model proposals for additional reusable signatures. Candidates must compile and validate against browser-local Essence execution before activation; model output never becomes the current request's canonical Essence. Reliable reusable-family repair is incomplete. |
