...
| Data-defined local semantic entities | Implemented foundation | `aws` | Versioned operation templates compile locally with Path bindings. Personal and qualified-owner quantities share signed observation storage. Quantity, participant-event, vocabulary-neutral activity occurrence, relationship transition, entity classification, selected-relation correction, and local trigger/effect policy entities exist. The relationship-transition contract separates an observed transition, object/reference identity, and a declaratively selected resulting state; classification can add a broader type to a specific entity without domain vocabulary. Correction provenance/version history, transitive ontology inference, contradictions, state expiration/reversal, and formal contract schemas remain incomplete. |
| Local syntax evidence + LLM semantic adjudication | Partial | `aws` + model services | The browser extracts vocabulary-neutral structural triplet evidence. A cold miss supplies it with bounded recent results and ContextDB to a strict model response containing speech act, answer or graph writes, grounded historical repairs, and Path plans. The browser requires those Paths to materialize the rows and reproduce the answer atomically; the model has no direct write authority. Contradiction policy, relevance selection, and broad cold-domain evaluation remain incomplete. See [decision 0012](../decisions/0012-llm-semantic-adjudication-to-locally-proven-paths.md). |
...
| Required and expanded Path learning | Partial | `aws` + model services | A cold miss performs semantic adjudication and builds current plus grounded historical repair plans; deterministic synthesis and the browser tournament validate, install, and replay them before graph execution. Candidate checks distinguish syntactic token coverage from typed semantic coverage, require adjudicated graph rows, and use an adjudicated question answer as a local proof target. Auto-build separately permits paraphrase signatures. General ontology inference, ambiguity confirmation, and broader dependency/capability-gap contracts remain incomplete. |
