...
| Data-defined local semantic entities | Implemented foundation | `aws` | Versioned operation templates compile locally with Path bindings. Personal and qualified-owner quantities share signed observation storage. Quantity, participant-event, vocabulary-neutral activity occurrence, selected-relation correction, and local trigger/effect policy entities exist. The activity contract separates actor, activity, object/kind, descriptor, modifier, measurement, and browser-local observation time so model-built Paths can reuse one graph shape across unrelated domains. Correction provenance/version history, ontology inference, and formal contract schemas remain incomplete. |
