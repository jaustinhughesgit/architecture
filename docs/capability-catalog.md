...
| Data-defined local semantic entities | Implemented foundation | `aws` | Versioned operation templates compile locally with Path bindings. Quantity/event observation, selected-relation correction, and local trigger/effect policy seeds exist; correction provenance/version history and formal contract schemas remain incomplete. |
