...
| Required and expanded Path learning | Partial | `aws` + model services | A cold miss builds a required candidate Path, then the browser validates, installs, and replays it before graph execution. Candidate checks now distinguish syntactic token coverage from explicit numeric semantic coverage. A query may require an atomic paired repair that removes and replays a defective supporting statement in isolation before the query is approved. Auto-build separately permits additional paraphrase signatures and cannot discard required repair proposals. General typed-role completeness, ontology inference, and immutable shared-family promotion remain incomplete. |
