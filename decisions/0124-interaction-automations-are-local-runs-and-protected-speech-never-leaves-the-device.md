...
Voice-originated proven ordinary responses create an ephemeral zero-delay speech run. The ordinary speech endpoint has no privacy selector and accepts only strict ordinary text plus the exact root interaction ID. It uses the explicitly configured OpenAI speech model first, returns transient MP3 bytes with `retainedByOnevar: false`, and records approximate configured model cost. The initiating microphone gesture prepares one reusable Web Audio context; successful MP3 bytes are decoded and played from that context after the network response arrives. Browser synthesis is a failure-only fallback rather than the normal path. The development release selects the established low-cost `tts-1` model with the `nova` voice. Typed responses remain silent.

Ordinary response delivery has separate preparation and release gates. Once the exact ordinary response text is stable, synthesis may start concurrently with remaining rendering, audit, workflow, and bookkeeping work. The browser holds the resulting audio under the exact interaction ID and response text. It releases playback only after an accepted result exists and all declared state changes required to support that result have committed. A clarification, changed projection, failed mutation, failed workflow step, cancellation, or new microphone capture aborts the preparation and cannot fall back to speaking stale text. No prepared audio is persisted.
...
- Exact local answers, authorized remote answers, and Compute answers can hide speech-generation latency without speaking before proof.
- Failed or superseded interactions do not leak a plausible but uncommitted spoken answer.
- Protected plaintext has no server TTS request schema, transport, log, or provider path.

## Verification

Contract tests reject unordered, oversized, and privacy-extended payloads. Browser tests prove persisted due-step execution, duration bounds, gesture-time Web Audio preparation, exact-text prefetch without early playback, one-request reuse at release, cancellation after release, generated MP3 decoding/playback, interruption, failure-only fallback, and a server-call-free protected path. API tests prove the exact reviewed OpenAI endpoint, selected model/voice configuration, and transient response contract.
