...
- Catalog policies may ground an adjacent typed modifier to a declared noun role and create its optional descriptor bindings, so separate noun phrases do not exchange attributes. When a model supplies an unavailable operation label, the compiler may canonicalize it only when the installed entity version, exact family, and mode identify one catalog operation; ambiguity fails closed.
