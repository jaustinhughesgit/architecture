...
- A relation-object policy may ground a typed role in the only noun following one of the catalog operation's declared relation markers. This prevents a model from collapsing an explicitly supplied destination or location to the actor while leaving multi-noun ambiguity for semantic adjudication.
- Catalog policies may ground an adjacent typed modifier to a declared noun role and create its optional descriptor bindings, so separate noun phrases do not exchange attributes. When a model supplies an unavailable operation label, the synthesis plan may retain its family as a non-executable routing hint and canonicalize it only when the installed entity version, exact family, and mode identify one catalog operation; ambiguity fails closed, and the emitted Path uses the catalog operation identity.
- A repair binding may declare ordered alternatives among existing typed source roles. The compiler uses the first available role, allowing a more specific role to win while safely falling back to an already-bound general role without another model guess.
- A catalog operation may expose one record through multiple declared observation relations when the record satisfies each contract. Relationship transitions are also observable activities and may retain an explicit quantity; one unambiguous prenominal digit or written cardinal is grounded by catalog policy while post-noun numeric identifiers remain untouched, allowing generic activity quantity queries to compose with transition statements without vocabulary code.
- An unavailable model operation may route through an installed entity version, mode, and either its exact catalog family or one uniquely longest catalog-family prefix followed by a decorative specialization suffix. The compiler emits only the canonical catalog identity; ambiguous prefixes fail closed.
- A query for a typed role already recorded on an occurrence traverses that occurrence record directly. It must not require the statement and question to independently invent the same natural-language relation label when the catalog's `object` and `state_subject` roles already define the join.
- When one occurrence is deliberately exposed through multiple catalog capabilities, a data-declared routing alias may redirect an unavailable model operation to exactly one installed cross-capability query. Data-declared first-available binding dependencies translate compatible typed role names; compact catalog transport must preserve both contracts, and ambiguous routes still fail closed.
