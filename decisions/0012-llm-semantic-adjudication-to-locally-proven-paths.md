...
10. All required Paths pass and commit together, or none commit. The ordinary answer becomes authoritative only after local reproduction succeeds. A separately labeled, unverified model `interpretation` may be shown with its explanation and correction status under [decision 0015](0015-visible-llm-interpretation-with-local-proof.md); it does not grant the model write authority or satisfy the local answer contract.
11. When an ordinary request matches multiple installed local question Paths, the browser evaluates all compatible candidates read-only against the same request-scoped ContextDB. A unique answer-bearing semantic interpretation executes locally regardless of learned-Path recency. Multiple distinct valid interpretations fail closed, and their bounded signatures, bindings, rows, and answers become model/clarification evidence; an empty set enters the same required repair transaction described above. A structurally matched candidate that failed its answer or transaction contract carries its typed request-time bindings as separate failed-match evidence—not successful coverage—so catalog routing can compare that role with exact executed roles in prior statements before the browser proves a replacement.
12. A catalog operation may authorize a fixed grammar token as semantic evidence only through a versioned `structuralTokenPolicy` whose declared effect bindings are satisfied. Numeric values and ordinary participants, entities, and descriptors still require typed bindings. Compatible wording extends the existing semantic family; repeated identical deterministic failures stop further model escalation. See [decision 0018](0018-catalog-authorized-structural-evidence-in-path-repair.md).
...
- An explicitly coordinated list is one repeated typed role. The model identifies the span and semantic role, typed promotion represents it as a reusable `entity_list` structural slot, local token processing extracts its noun members, and a versioned catalog loop owns the graph expansion. A candidate that captures only the first member fails semantic completeness; core code contains no vocabulary from the motivating list.
- Model answers are useful proof targets and visible diagnostics but never unverified runtime answers.
...
- **Trust and display the model answer immediately as the ordinary answer.** Rejected because the installed Path and local graph may still be unable to reproduce it. A clearly labeled diagnostic interpretation is allowed by decision 0015.
