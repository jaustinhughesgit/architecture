...
9. When a supporting statement is replayed for isolated proof or live commit, the browser preserves its recorded graph identifier only for bindings typed as `resolvedEntity`; live cleanup also retains that node. Local re-tokenization remains authoritative for every other binding. This prevents a repaired statement from creating a duplicate node that separates an object's classification from its properties.
