...
9. When a supporting statement is replayed for isolated proof or live commit, the browser preserves its recorded graph identifier only for token-sourced bindings typed as `resolvedEntity`; live cleanup also retains that node. `currentSpeaker`, binding-derived roles, and local re-tokenization remain authoritative for every other binding. This prevents a repaired statement from creating a duplicate object while also preventing stale pronoun nodes from replacing canonical speaker identity.
