...
Ask opens Message and may queue only the fixed, non-protected approval notice. The opened Message panel retains the Speak and Reveal dropdowns plus their one-time authorization buttons; the notice never replaces those controls. A reference-bearing protected result renders this request area even if a redundant boolean marker is absent. Don't ask keeps the answer masked without that notice. A microphone press interrupts the notice under the same capture-preemption rule.
