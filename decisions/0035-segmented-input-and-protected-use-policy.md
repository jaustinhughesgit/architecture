...
Unit tests cover provisional-capture cancellation, four-lane mode selection, exact `ordinary → protected → ordinary` sample continuity and ordering, browser-local protected-worker transfer, recent-input redaction, release-time assembly markers, policy mapping, executor-wrap selection, and server policy validation. Browser QA covers the actual AudioWorklet frame boundary and browser-local model initialization/inference. Physical microphone pronunciation accuracy, wider device coverage, and long-duration performance remain acceptance work.
