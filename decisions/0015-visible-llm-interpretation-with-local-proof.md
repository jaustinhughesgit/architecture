...
4. Message includes `interpretation` in `msgResult`. When no local answer exists, `msgAnswer` may show `interpretation.answer` only with an explicit LLM/unverified status and explanation. The authoritative `message.answer` field remains empty until local execution returns an answer.
