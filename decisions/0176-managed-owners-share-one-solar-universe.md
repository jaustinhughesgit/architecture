...
The return variant pulls its middle and inner rings toward the back icon. User and
managed-owner solar systems use solid white boundaries in open and collapsed states;
a closed managed system is not drawn as a wormhole. This presentation-only revision
replaces the initial literal `~` marks. No identity or navigation contract changes.
...
component. Entry and return symbols use three fading circles and eight curved ribs;
all existing navigation, actor/owner, protected and marketplace
...
34978441239 subsequently passed its full repository regression. No production
deployment or existing-user data reset occurred.

The follow-up presentation standard removes the decorative ring from collapsed
managed systems and gives all user/managed system boundaries one solid white circle.
The 60-pixel return control uses 150/112 middle/inner radii so its warp reaches
toward the back icon. Filled entity labels use weight 400 at the next smaller size;
exact `#e5bf47`/`#cad050` fills use black and every other Sunburst fill uses white.
Sharing projection follows the same boundary and typography rules. This changes no
owner identity, camera, graph, permissions or protected/marketplace authority.
