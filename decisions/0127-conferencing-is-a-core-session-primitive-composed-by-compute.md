...
An authorized public profile label may nominate an invitee, but the API must resolve and freeze one exact entity ID before creating the invitation. Acceptance creates participant authority. Provider-token issuance records `authorized`; only a separate exact acknowledgment after the browser media session starts advances that participant to `joined`. The service may retain value-free joined/left liveness and the call surface may check it on a bounded in-call cadence, but no media, device detail, or provider token is retained. A session ID, presence record, app focus, Path, or name cannot substitute for authority. Only the organizer may terminate the complete session.

The browser call component remains mounted while app and Sunburst projections change. Leave, organizer end, remote end, setup failure, and page teardown converge on one idempotent browser cleanup. That cleanup explicitly stops selected camera and microphone inputs, stops bound tracks, unbinds elements, removes observers, and then stops the provider session; it repeats after an in-flight device-selection promise settles to close late-permission races. A closed surface or provider-session stop alone is not proof that the browser released media. Compute entities may later schedule a consultation, invite a participant, charge for a service, or run post-call work by invoking reviewed conference operations; they do not regenerate signaling, credential, or media logic.
