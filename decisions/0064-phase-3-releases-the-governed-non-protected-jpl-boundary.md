...
- Convert freezes an answer plan before proposing a typed fixed-transition contract. Trusted canonicalization guarantees one unambiguous required selected-subject input even when several model fields are equally plausible; trusted projection restores its frozen name, type, required flag, and dependency ownership, and makes the frozen transition endpoints authoritative over repeated contract-stage fields.
- Each model stage may replace one response that ends in a recognized transient provider failure. Revision-checked durable state prevents parallel replacement starts, every response ID and available token receipt—including a completed response later rejected by validation—is retained, and semantic or validation failures are never retried.
