...
The owner ID constrains identity and response perspective. The owned target ID constrains the Compute dependency and effect subject. Possessive morphology is normalized only for structural matching; the captured surface remains available for presentation. For Position retrieval, a possessive imperative normalizes the owner perspective to the same deictic identity slot used by the reusable command (`Wash Austin's car` becomes `wash my car`). It does not remove the identity slot or add Austin to the shared behavior. The shared manifest, Position, and creator Path do not acquire either caller-specific ID.
