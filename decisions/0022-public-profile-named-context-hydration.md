...
- The browser installs the resolved target's public display name on its stable node, while stripping remote current-speaker aliases. Only the authenticated requesting principal maps to local `speaker`. When the hydrated graph contains both the named subject and its same-labelled literal name object, local mention resolution follows the authorized `name`, `display name`, or `full name` edge to the unique subject and passes that exact entity ID into the Path. Multiple matching owners remain ambiguous.
...
- Browser tests extract Austin only from proper-person syntax, prevent the remote user from becoming local `speaker`, retry a unique exact-name miss until the asynchronous publisher appears, stop immediately on ambiguity, resolve a same-labelled name subject and literal through their graph edge, call the exact-name action, and run the existing local Essence query over the hydrated graph.
