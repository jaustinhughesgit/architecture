...
The original implementation stored active schedules and retryable occurrences in one sixteen-shard DynamoDB `ScheduleDueIndex`. [Decision 0113](0113-time-bucketed-due-pointers-scale-schedule-dispatch.md) supersedes that physical projection with versioned minute-bucketed due pointers while preserving this decision's exact claim and execution contract. One EventBridge one-minute cadence claims due schedules atomically and creates deterministic occurrence and Compute invocation identities. SQS separates dispatch from execution, supports partial-batch failure, and redrives infrastructure failures to a DLQ. Domain retries retain the exact occurrence and invocation IDs, so provider idempotency, effect proof, and credit settlement cannot duplicate.
...
Results enter a durable browser inbox. Ordinary Compute results rejoin the exact active local installation before its declared Context effects and response template run. Relation identity is frozen, while delivery may refresh only that already-bound relation ID to its current version before rechecking subject, ownership, property compatibility, and the declared expected/current value contract. A missing, replaced, or incompatible relation fails closed. This admits legitimate same-relation evolution between preparation and delivery without allowing name-based rebinding. Each inbox occurrence is isolated so one result that remains pending cannot stop later committed results from being delivered. Protected provider results remain executor-encrypted for the exact browser recipient key. Execution commit and browser delivery acknowledgement are distinct and user-visible. Pause, resume, cancel, retry, history, failure, and acknowledgement use stable IDs.
