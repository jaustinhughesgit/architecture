...
- declaring a required binding name is not semantic coverage unless its source can materialize a concrete value. Null or empty required roles fail catalog compilation and isolated execution, and historical repair treats an empty stored binding value as missing evidence;
