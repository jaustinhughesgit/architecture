...
- The browser does not call Compute directly for this bootstrap. It posts to the website's same-origin `/newuser/create` relay. The relay calls the exact Compute route, accepts only the declared response envelopes, requires the replacement token plus its user/workspace pair, and installs that token as a first-party domain-scoped HttpOnly cookie. It returns only the user and workspace IDs to page JavaScript.
...
Opening `/newuser` intentionally starts a new browser identity even when the browser has a valid prior session. This matches the page's purpose and makes cold-reset and multi-browser tests repeatable. The first-party relay prevents browser cookie-partition policy from acknowledging one workspace to the page while declining the matching cross-origin cookie. Navigating to ordinary workspaces never rotates identity. Context workspace verification remains fail closed.

## Affected repositories

- `aws`: owns the first-party new-user relay, requests the fresh identity handshake, installs the acknowledged HttpOnly token, and retains the post-reset defensive cookie expiry.
...
The flag grants no access to an existing account and cannot weaken another route's authorization. It creates a new anonymous identity by explicit navigation to the new-user surface. Existing identity cookies are expired. The relay handles the replacement token only on the server, installs it HttpOnly, and never logs it or returns it in response JSON.
...
- Website contract tests require same-origin `/newuser/create`, the exact upstream flag, declared envelope unwrapping, final non-empty token selection, and an HttpOnly cookie response that exposes only user/workspace IDs.
