...
After semantic discovery has validated one operation with one fixed replacement effect, Compute may compile the local declarative execution without a second model call. This deterministic path is available only when every required input is the effect subject or an exact same-name response projection and at most one remaining string output is the answer plan's declared literal result; the effect's fixed new value supplies that output. Multiple unresolved outputs, protected requirements, provider-backed answers, calculations, and unused required inputs fail this deterministic test and continue through the separately validated EntityPlan lifecycle. Discovery still owns semantic generalization; the compiler only materializes an already-proved contract.

The governed entity boundary derives that same result directly from the validated operation before invoking legacy JPL: exact same-name outputs echo their typed inputs, and the one unresolved string result receives the fixed new value. This does not apply the graph change on the server; browser-main or the delegated effect endpoint remains the mutation authority. If the operation is not equally unambiguous at runtime, execution falls back to its registered implementation. The contract therefore remains executable even when loading a redundant local JPL would add latency without adding semantics.

The browser emits an owner-local mutation through the existing graph-operation and persistence path. Before presenting success, it applies the requested operation and verifies that every declared target relation now points to the declared result value. A failed postcondition restores the pre-invocation local graph and returns a typed failure with no mutation receipt or success answer. A successful result includes a bounded receipt derived from the locally verified relation changes. Effect metadata is evidence, not a second Essence fact, so it must not be ingested again after the rewire. Ordinary publication remains asynchronous and governed by its existing audience and authorization contract; publication compares relation contents as well as IDs so a same-ID object rewire is synchronized. When that local ID is already an acknowledged canonical relation ID, the server preserves it only after verifying that the relation exists in the authenticated publisher's audience and belongs to that publisher. The new object is then a later version of the same relation instead of an additional relation beside the old value; unknown or foreign canonical-looking IDs cannot claim an existing relation. A named foreign relation never passes through ordinary caller publication. An owner-published capability may instead authorize its exact declared effect through the narrower delegated contract in [decision 0051](0051-owner-published-compute-delegates-bounded-context-effects.md).
...
- JPL remains responsible for computing and returning the declared value; browser-main remains responsible for local proof, while the owner-published delegated service endpoint owns the bounded server commit for a foreign relation.
- A fixed transition no longer depends on a redundant model-authored implementation when the validated effect, inputs, and one result determine the complete declarative JPL response.
- The same fixed transition no longer depends on legacy JPL startup to return its already-declared typed result.
...
- Publication tests prove a same-ID relation rewire is emitted and retains its authenticated acknowledged identity, and runtime tests require the postcondition before returning success.
