...
Database reset is authorized by Compute, not by possession of the testing client. Compute requires an explicit test-stack enable flag and exact non-production environment identity. Deployments normally require an authenticated user from an explicit allow-list. During the current disposable-stack testing phase, the existing any-user compatibility switch may deliberately admit anonymous portal callers; this temporary exception remains subordinate to enablement and exact-environment checks. Client checks add protection against mistakes but are not a security boundary.
