...
Typed discovery and replay preserve an explicit relative-day surface even when the model also supplied its normalized date. A semantically named date field receives the same local treatment if generated metadata omits a redundant date type or resolver. One explicit relative-day surface is authoritative across a UTC calendar rollover; multiple possible surfaces remain ambiguous. This keeps Path learning grounded in words the user actually said while allowing execution to receive the canonical date locally.
