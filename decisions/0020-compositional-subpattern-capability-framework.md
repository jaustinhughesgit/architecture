...
A network component may be optional when the catalog binding already defines the only safe omitted meaning. The quantity statement uses this for an owner omitted by conversational speech or transcription: skipping the owner component retains `currentSpeaker`; consuming an explicit owner activates the existing owner alternative and replaces that binding. Optionality does not guess an unnamed third party and does not relax complete coverage for the tokens that are present.
