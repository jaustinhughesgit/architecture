...
Speech-act proof is operation-specific as well as punctuation-aware. In particular, an owned-object list needs an explicit read cue; a declarative first-person possession cannot be accepted as a list just because its wording contains `count`, `things`, or `own`. The same guard applies to warm template matching so an older broad read template cannot intercept a new fact.
