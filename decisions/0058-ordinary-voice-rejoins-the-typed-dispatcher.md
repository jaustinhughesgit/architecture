...
The transcript is replayed through the same command dispatcher used by typed input, producing the same Interaction Evidence, Essence, Path, Compute, and response behavior. Decision 0065 replaces the temporary three-position Phase 3 projection with four purpose/trust lanes. Ordinary Essence is position 2 and ordinary Convert is position 3; a Convert-started 3 → 2 → 3 excursion remains the hard-stop gesture and final build submits the same ordered Convert contract as typed `[hard stop]` input.
